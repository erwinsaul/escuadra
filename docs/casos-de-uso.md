...
## CU-Geo-01: Calcular propiedades geométricas de una figura

**Actor:** Estudiante de ingeniería

**Descripción:** El usuario selecciona una figura geométrica (triángulo, círculo, rectángulo, etc.), ingresa sus dimensiones y obtiene propiedades como área, perímetro o volumen.
...
**Postcondición:** Las propiedades geométricas de la figura son calculadas y mostradas al usuario, con la posibilidad de reiniciar el cálculo con nuevos valores.

## CU-Geo-02: Calcular área de figura geométrica
...
## CU-Elec-01: Analizar un circuito eléctrico básico

**Actor:** Estudiante de ingeniería

**Descripción:** El usuario ingresa los componentes de un circuito (resistencias, voltaje, corriente) y el sistema calcula magnitudes como resistencia equivalente, corriente total y caída de tensión, aplicando las leyes de Ohm y Kirchhoff.

**Precondición:** El usuario tiene acceso al módulo eléctrico y conoce los valores de los componentes del circuito a analizar.
...
- **A1 (valor de componente inválido):** Si el usuario ingresa un valor no numérico, negativo o igual a cero en algún componente, el sistema muestra un mensaje de error y no realiza el cálculo.
- **A2 (circuito incompleto):** Si el usuario intenta calcular sin haber definido al menos un componente o la fuente de tensión, el sistema solicita completar los datos antes de continuar.
...
## CU-Civ-01: Verificar momento máximo en viga
...
## CU-Mat-01: Resolver un sistema de ecuaciones lineales

**Actor:** Estudiante de ingeniería / Docente

**Descripción:** El usuario ingresa los coeficientes de un sistema de ecuaciones lineales y el sistema devuelve la solución mediante métodos como Gauss-Jordan o sustitución.

**Precondición:** El usuario tiene acceso al módulo de matemáticas y conoce los coeficientes del sistema de ecuaciones a resolver.
...
- **A1 (sistema sin solución única):** Si el sistema de ecuaciones no tiene solución única (es inconsistente o indeterminado), el sistema indica esta condición en lugar de mostrar un resultado numérico.
- **A2 (coeficiente no numérico):** Si el usuario ingresa un coeficiente no numérico, el sistema muestra un mensaje de error y no realiza el cálculo hasta que el valor sea corregido.

**Postcondición:** La solución del sistema de ecuaciones es calculada y mostrada al usuario junto con el procedimiento paso a paso.

## CU-Mat-02: Convertir unidades de medida entre sistemas

**Actor:** Estudiante de ingeniería

**Descripción:** El usuario ingresa un valor con su unidad de origen y selecciona la unidad destino. El sistema realiza la conversión de forma inmediata (longitud, masa, presión, temperatura, etc.).

**Precondición:** El usuario tiene acceso al módulo de matemáticas y conoce el valor y la unidad de origen a convertir.
...
- **A1 (valor no numérico):** Si el usuario ingresa un valor no numérico, el sistema muestra un mensaje de error y no realiza la conversión.
- **A2 (unidades incompatibles):** Si el usuario selecciona una unidad destino que no corresponde a la misma categoría que la unidad de origen (por ejemplo, convertir longitud a temperatura), el sistema impide la conversión y solicita seleccionar una unidad válida.

**Postcondición:** El valor ingresado es convertido correctamente a la unidad destino y mostrado al usuario.

---

## General

## CU-Gen-01: Consultar fórmulas técnicas por área de ingeniería

**Actor:** Docente / Estudiante de ingeniería

**Descripción:** El usuario navega por un repositorio de fórmulas organizadas por rama de ingeniería (civil, mecánica, sistemas, etc.) para consultar, entender y aplicar las expresiones matemáticas relevantes a su área.

**Precondición:** El usuario tiene acceso al módulo de referencia técnica.
...
- **A1 (rama sin fórmulas disponibles):** Si la rama de ingeniería seleccionada aún no tiene fórmulas documentadas, el sistema indica que el contenido está pendiente en lugar de mostrar una lista vacía sin explicación.

**Postcondición:** El usuario obtiene la descripción, variables y ejemplo de aplicación de la fórmula consultada, y puede continuar hacia una herramienta de cálculo relacionada.
